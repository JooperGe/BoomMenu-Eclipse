...
public class MainActivity extends Activity implements
		...
		BoomMenuButton.AnimatorListener, View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boomMenuButtonInActionBar = (BoomMenuButton) mCustomView
				.findViewById(R.id.boom);
		...
		boomInfo.setOnSubButtonClickListener(new BoomMenuButton.OnSubButtonClickListener() {
			...
			public void onClick(int buttonIndex) {
				if (buttonIndex == 0) {
					Toast.makeText(mContext, "Boom!", Toast.LENGTH_SHORT)
							.show();
				} else if (buttonIndex == 1) {
					startActivity(new Intent(Intent.ACTION_VIEW, Uri
							.parse("https://github.com/Nightonke/BoomMenu")));
				} else if (buttonIndex == 2) {
					startActivity(new Intent(Intent.ACTION_VIEW, Uri
							.parse("https://github.com/Nightonke")));
				}
			}
		});
	...
	private void initInfoBoom() {
		...
		int[] drawablesResource = new int[] { R.drawable.boom, R.drawable.java,
				R.drawable.github };
		for (int i = 0; i < 3; i++)
			drawables[i] = getResources().getDrawable(drawablesResource[i]);

		int[][] colors = new int[3][2];
		for (int i = 0; i < 3; i++) {
			colors[i][1] = getResources().getColor(R.color.material_white);
			colors[i][0] = Util.getInstance().getPressedColor(colors[i][1]);
		}

		boomInfo.init(drawables, new String[] { "BoomMenuButton",
				"View source code", "Follow me" }, colors, ButtonType.HAM,
				BoomType.PARABOLA, PlaceType.HAM_3_1, null, null, null, null,
				null, null, null);

		boomInfo.setSubButtonShadowOffset(Util.getInstance().dp2px(2), Util
				.getInstance().dp2px(2));

		boomInfo.setTextViewColor(getResources().getColor(R.color.black));
	...
	private void initBoom() {
		...
		int[] drawablesResource = new int[] { R.drawable.mark,
				R.drawable.refresh, R.drawable.copy, R.drawable.heart,
				R.drawable.info, R.drawable.like, R.drawable.record,
				R.drawable.search, R.drawable.settings };
		for (int i = 0; i < number; i++)
			drawables[i] = getResources().getDrawable(drawablesResource[i]);

		String[] STRINGS = new String[] { "Mark", "Refresh", "Copy", "Heart",
				"Info", "Like", "Record", "Search", "Settings" };
		...
		boomMenuButton.init(drawables, strings, colors, buttonType,
				getBoomType(), getPlaceType(), null, null, null, null, null,
				null, null);

		boomMenuButtonInActionBar.init(drawables, strings, colors, buttonType,
				getBoomType(), getPlaceType(), null, null, null, null, null,
				null, null);

		boomMenuButton.setSubButtonShadowOffset(Util.getInstance().dp2px(2),
				Util.getInstance().dp2px(2));
		boomMenuButtonInActionBar.setSubButtonShadowOffset(Util.getInstance()
				.dp2px(2), Util.getInstance().dp2px(2));
	}
	...
	private void initViews() {
		...
		animationDurationSeek
				.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar,
							int progress, boolean fromUser) {
						animationDurationText.setText(((progress + 1) * 500)
								+ " ms");
						boomMenuButton.setDuration((progress + 1) * 500);
						boomMenuButtonInActionBar
								.setDuration((progress + 1) * 500);
					}
		...
		animationDurationText
				.setText(((animationDurationSeek.getProgress() + 1) * 500)
						+ " ms");
		boomMenuButton
				.setDuration((animationDurationSeek.getProgress() + 1) * 500);
		boomMenuButtonInActionBar.setDuration((animationDurationSeek
				.getProgress() + 1) * 500);
		...
		animationStartDelaySeek
				.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar,
							int progress, boolean fromUser) {
						animationStartDelayText.setText(((progress + 1) * 100)
								+ " ms");
						boomMenuButton.setDelay((progress + 1) * 100);
						boomMenuButtonInActionBar
								.setDelay((progress + 1) * 100);
					}
		...
		animationStartDelayText
				.setText(((animationStartDelaySeek.getProgress() + 1) * 100)
						+ " ms");
		boomMenuButton
				.setDelay((animationStartDelaySeek.getProgress() + 1) * 100);
		boomMenuButtonInActionBar.setDelay((animationStartDelaySeek
				.getProgress() + 1) * 100);
		...
		animationRotationDegreeSeek
				.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar,
							int progress, boolean fromUser) {
						animationRotationDegreeText.setText((progress * 360)
								+ " degrees");
						boomMenuButton.setRotateDegree(progress * 360);
						boomMenuButtonInActionBar
								.setRotateDegree(progress * 360);
					}
		...
		animationRotationDegreeText.setText((animationRotationDegreeSeek
				.getProgress() * 360) + " degrees");
		boomMenuButton.setRotateDegree(animationRotationDegreeSeek
				.getProgress() * 360);
		boomMenuButtonInActionBar.setRotateDegree(animationRotationDegreeSeek
				.getProgress() * 360);
				...
				.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
					...
					public void onCheckedChanged(RadioGroup group, int checkedId) {
						...
						case R.id.click_effect_ripple:
							boomMenuButton
									.setClickEffectType(ClickEffectType.RIPPLE);
							boomMenuButtonInActionBar
									.setClickEffectType(ClickEffectType.RIPPLE);
						...
						case R.id.click_effect_normal:
							boomMenuButton
									.setClickEffectType(ClickEffectType.NORMAL);
							boomMenuButtonInActionBar
									.setClickEffectType(ClickEffectType.NORMAL);
	...
	private String[] Colors = { "#F44336", "#E91E63", "#9C27B0", "#2196F3",
	...
	public void onClick(int buttonIndex) {
		Toast.makeText(
				this,
				"On click "
						+ boomMenuButton.getTextViews()[buttonIndex].getText()
								.toString() + " button", Toast.LENGTH_SHORT)
				.show();
	}
	...
	public void onBackPressed() {
		if (boomMenuButton.isClosed() && boomMenuButtonInActionBar.isClosed()
				&& boomInfo.isClosed()) {
			super.onBackPressed();
		} else {
