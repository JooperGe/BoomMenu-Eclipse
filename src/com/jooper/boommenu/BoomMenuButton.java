...
import com.jooper.boommenu.Eases.EaseType;
import com.jooper.boommenu.Types.BoomType;
import com.jooper.boommenu.Types.ButtonType;
import com.jooper.boommenu.Types.ClickEffectType;
import com.jooper.boommenu.Types.DimType;
import com.jooper.boommenu.Types.OrderType;
import com.jooper.boommenu.Types.PlaceType;
import com.jooper.boommenu.Types.StateType;
...
public class BoomMenuButton extends FrameLayout implements
	...
	public BoomMenuButton(Context context, AttributeSet attrs) {
		...
		TypedArray attr = context.obtainStyledAttributes(attrs,
				R.styleable.BoomMenuButton, 0, 0);
		if (attr != null) {
			try {
				isInActionBar = attr.getBoolean(
						R.styleable.BoomMenuButton_boom_inActionBar, false);
				// boomButtonColor = attr.getColor(
				// R.styleable.BoomMenuButton_boom_button_color,
				// ContextCompat.getColor(mContext,
				// R.color.default_boom_button_color));
				// boomButtonPressedColor = attr.getColor(
				// R.styleable.BoomMenuButton_boom_button_pressed_color,
				// ContextCompat.getColor(mContext,
				// R.color.default_boom_button_color_pressed));
			} finally {
				attr.recycle();
			}
		}
		...
		if (isInActionBar) {
			LayoutInflater.from(context).inflate(
					R.layout.boom_menu_button_in_action_bar, this, true);
			...
			if (false) {
				LayoutInflater.from(context).inflate(R.layout.boom_menu_button,
						this, true);
			} else {
				LayoutInflater.from(context).inflate(
						R.layout.boom_menu_button_below_lollipop, this, true);
			}
			...
			setBoomButtonBackgroundColor(boomButtonPressedColor,
					boomButtonColor);
		}

		hamButtonWidth = (int) (Util.getInstance().getScreenWidth(getContext()) * 8 / 9 + Util
				.getInstance().dp2px(4));
	 ...
	 * Call this in:
	 *
	 * (This method needs to be overrided in activity) public void
	 ...
	 * @param colors
	 *            The colors of sub buttons, including pressed-state and
	 *            normal-state.
	 * @param buttonType
	 *            The button type.
	 * @param boomType
	 ...
	 * @param placeType
	 *            The place type.
	 * @param showMoveEaseType
	 *            Ease type to move the sub buttons when showing.
	 * @param showScaleEaseType
	 *            Ease type to scale the sub buttons when showing.
	 * @param showRotateEaseType
	 *            Ease type to rotate the sub buttons when showing.
	 * @param hideMoveEaseType
	 *            Ease type to move the sub buttons when dismissing.
	 * @param hideScaleEaseType
	 *            Ease type to scale the sub buttons when dismissing.
	 * @param hideRotateEaseType
	 *            Ease type to rotate the sub buttons when dismissing.
	 * @param rotateDegree
	 *            Rotation degree.
	 */
	public void init(Drawable[] drawables, String[] strings, int[][] colors,
			ButtonType buttonType, BoomType boomType, PlaceType placeType,
			EaseType showMoveEaseType, EaseType showScaleEaseType,
			EaseType showRotateEaseType, EaseType hideMoveEaseType,
			EaseType hideScaleEaseType, EaseType hideRotateEaseType,
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			...
			for (int i = 0; i < buttonNum; i++) {
				...
				if (strings != null)
					circleButtons[i].setText(strings[i]);
			...
			for (int i = 0; i < buttonNum; i++) {
				...
				if (strings != null)
					hamButtons[i].setText(strings[i]);
	 ...
	 *
	 * @param drawables
	 *            The drawables of the sub buttons.
	 * @param strings
	 *            The texts of the sub buttons.
	 * @param colors
	 *            The colors(including the pressed-state and normal-state) of
	 *            the sub buttons.
	 * @param buttonType
	 *            The button type of the sub buttons.
	 */
	private void errorJudge(Drawable[] drawables, String[] strings,
			int[][] colors, ButtonType buttonType) {
		if (drawables == null) {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			if (!(MIN_CIRCLE_BUTTON_NUMBER <= drawables.length && drawables.length <= MAX_CIRCLE_BUTTON_NUMBER)
					|| (strings != null && !(MIN_CIRCLE_BUTTON_NUMBER <= strings.length && strings.length <= MAX_CIRCLE_BUTTON_NUMBER))
					|| !(MIN_CIRCLE_BUTTON_NUMBER <= colors.length && colors.length <= MAX_CIRCLE_BUTTON_NUMBER)) {
				throw new RuntimeException(
						"The circle type button's length must be in ["
								+ MIN_CIRCLE_BUTTON_NUMBER + ", "
								+ MAX_CIRCLE_BUTTON_NUMBER + "]!");
			}
		} else if (buttonType.equals(ButtonType.HAM)) {
			if ((!(MIN_HAM_BUTTON_NUMBER <= drawables.length && drawables.length <= MAX_HAM_BUTTON_NUMBER))
					|| (strings != null && !(MIN_HAM_BUTTON_NUMBER <= strings.length && strings.length <= MAX_HAM_BUTTON_NUMBER))
					|| !(MIN_HAM_BUTTON_NUMBER <= colors.length && colors.length <= MAX_HAM_BUTTON_NUMBER)) {
				throw new RuntimeException(
						"The ham type button's length must be in ["
	...
	private void dimAnimationLayout() {
		...
		ObjectAnimator objectAnimator = ObjectAnimator.ofInt(animationLayout,
				"backgroundColor", DimType.DIM_0.value, dimType.value)
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			...
			if (showOrderType.equals(OrderType.DEFAULT)) {
				for (int i = 0; i < buttonNum; i++) {
					...
					setShowAnimation(dots[i], circleButtons[i],
				...
				for (int i = 0; i < buttonNum; i++) {
					...
					startLocations[i][1] -= (buttonWidth - dots[i].getHeight()) / 2;
					setShowAnimation(dots[i], circleButtons[i],
							startLocations[i], endLocations[i], buttonNum - i
									- 1);
				}
				...
				while (true) {
					...
					if (!used[i]) {
						...
						startLocations[count][0] -= (buttonWidth - dots[count]
								.getWidth()) / 2;
						startLocations[count][1] -= (buttonWidth - dots[count]
								.getHeight()) / 2;
						setShowAnimation(dots[count], circleButtons[count],
								startLocations[count], endLocations[count], i);
			...
			if (showOrderType.equals(OrderType.DEFAULT)) {
				for (int i = 0; i < buttonNum; i++) {
					...
					startLocations[i][0] -= (hamButtonWidth - bars[i]
							.getWidth()) / 2;
					startLocations[i][1] -= (hamButtonHeight - bars[i]
							.getHeight()) / 2;
					setShowAnimation(bars[i], hamButtons[i], startLocations[i],
							endLocations[i], i);
				}
			} else if (showOrderType.equals(OrderType.REVERSE)) {
				for (int i = 0; i < buttonNum; i++) {
				...
				while (true) {
					...
					if (!used[i]) {
						...
						startLocations[count][0] -= (hamButtonWidth - bars[count]
								.getWidth()) / 2;
						startLocations[count][1] -= (hamButtonHeight - bars[count]
								.getHeight()) / 2;
						setShowAnimation(bars[count], hamButtons[count],
								startLocations[count], endLocations[count], i);
	...
	private void getEndLocations() {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			...
			case 4:
				if (placeType.equals(PlaceType.CIRCLE_4_1)) {
					endLocations[0][0] = width / 3 - buttonWidth / 2;
					endLocations[0][1] = height / 2 - width / 6 - buttonWidth
							/ 2;
					endLocations[1][0] = width * 2 / 3 - buttonWidth / 2;
					endLocations[1][1] = height / 2 - width / 6 - buttonWidth
							/ 2;
					endLocations[2][0] = width / 3 - buttonWidth / 2;
					endLocations[2][1] = height / 2 + width / 6 - buttonWidth
							/ 2;
					endLocations[3][0] = width * 2 / 3 - buttonWidth / 2;
					endLocations[3][1] = height / 2 + width / 6 - buttonWidth
							/ 2;
				}
				if (placeType.equals(PlaceType.CIRCLE_4_2)) {
					double s2 = Math.sqrt(2);
			...
			case 6:
				...
				if (placeType.equals(PlaceType.CIRCLE_6_4)) {
					...
					endLocations[0][1] = height / 2 - dis1 - buttonWidth / 2;
					endLocations[1][0] = width / 2 + dis2 - buttonWidth / 2;
					endLocations[1][1] = height / 2 - dis1 / 2 - buttonWidth
							/ 2;
					endLocations[2][0] = width / 2 + dis2 - buttonWidth / 2;
					endLocations[2][1] = height / 2 + dis1 / 2 - buttonWidth
							/ 2;
					endLocations[3][0] = width / 2 - buttonWidth / 2;
					endLocations[3][1] = height / 2 + dis1 - buttonWidth / 2;
					endLocations[4][0] = width / 2 - dis2 - buttonWidth / 2;
					endLocations[4][1] = height / 2 + dis1 / 2 - buttonWidth
							/ 2;
					endLocations[5][0] = width / 2 - dis2 - buttonWidth / 2;
					endLocations[5][1] = height / 2 - dis1 / 2 - buttonWidth
							/ 2;
				}
				if (placeType.equals(PlaceType.CIRCLE_6_5)) {
			...
			case 7:
				...
				if (placeType.equals(PlaceType.CIRCLE_7_4)) {
					...
					endLocations[2][1] = height / 2 - dis1 / 2 - buttonWidth
							/ 2;
					endLocations[3][0] = width / 2 + dis2 - buttonWidth / 2;
					endLocations[3][1] = height / 2 + dis1 / 2 - buttonWidth
							/ 2;
					endLocations[4][0] = width / 2 - buttonWidth / 2;
					endLocations[4][1] = height / 2 + dis1 - buttonWidth / 2;
					endLocations[5][0] = width / 2 - dis2 - buttonWidth / 2;
					...
					endLocations[6][1] = height / 2 - dis1 / 2 - buttonWidth
							/ 2;
				}
				break;
			case 8:
			...
			case 9:
				...
				if (placeType.equals(PlaceType.CIRCLE_9_2)) {
					...
					endLocations[7][1] = height / 2 + dis1 / 2 - buttonWidth
							/ 2;
	...
	/**
	 * Create the background layout as a "canvas" of all animations and sub
	 * buttons. Notice that we don't need to call this every time, we can set
	 * the visibility of the background layout to hide it.
	 *
	 ...
	 */
	private View setViewLocationInAnimationLayout(final View view,
			int[] location) {
		int x = location[0];
		int y = location[1];
	 ...
	 *
	 * @param dot
	 *            The dot corresponding to the sub button.
	 * @param button
	 *            The sub button.
	 * @param startLocation
	 *            Start location of the animation.
	 * @param endLocation
	 *            End location of the animation.
	 * @param index
	 *            Index of the sub button in the array.
	 */
	@SuppressLint("NewApi")
	public void setShowAnimation(final View dot, final View button,
			int[] startLocation, int[] endLocation, final int index) {
		button.bringToFront();

		final View view = setViewLocationInAnimationLayout(button,
				startLocation);
		...
		ObjectAnimator xAnimator = ObjectAnimator.ofFloat(view, "x", xs)
				.setDuration(duration);
		xAnimator.setStartDelay(delay * index);
		xAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(showMoveEaseType));
		xAnimator.start();

		ObjectAnimator yAnimator = ObjectAnimator.ofFloat(view, "y", ys)
				.setDuration(duration);
		yAnimator.setStartDelay(delay * index);
		yAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(showMoveEaseType));
		yAnimator.start();
		...
		ObjectAnimator scaleXAnimator = ObjectAnimator.ofFloat(view, "scaleX",
				scaleW, 1f).setDuration(duration);
		scaleXAnimator.setStartDelay(delay * index);
		scaleXAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(showScaleEaseType));
		scaleXAnimator.start();

		view.setScaleY(scaleH);
		ObjectAnimator scaleYAnimator = ObjectAnimator.ofFloat(view, "scaleY",
				scaleH, 1f).setDuration(duration);
		scaleYAnimator.setStartDelay(delay * index);
		scaleYAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(showScaleEaseType));
		...
		alphaAnimator1.setInterpolator(InterpolatorFactory
				.getInterpolator(showMoveEaseType));
		...
		alphaAnimator2.setInterpolator(InterpolatorFactory
				.getInterpolator(showMoveEaseType));
		...
		if (view instanceof CircleButton) {
			ObjectAnimator rotateAnimator = ObjectAnimator.ofFloat(
					((CircleButton) view).getFrameLayout(), "rotation", 0,
					rotateDegree).setDuration(duration);
			rotateAnimator.setStartDelay(delay * index);
			rotateAnimator.setInterpolator(InterpolatorFactory
					.getInterpolator(showRotateEaseType));
	...
	/**
	 * Get the function of the road of the animation of showing. Then calculate
	 * each points to be ready for the animation.
	 *
	 * @param startPoint
	 ...
	 */
	private void getShowXY(float[] startPoint, float[] endPoint, float[] xs,
			float[] ys) {
		if (boomType.equals(BoomType.LINE)) {
			...
			float y2 = endPoint[1];
			float x3 = (startPoint[0] + endPoint[0]) / 2;
			float y3 = (Util.getInstance().getScreenHeight(mContext) - Math
	...
	/**
	 * Get the function of the road of the animation of dismissing. Then
	 * calculate each points to be ready for the animation.
	 *
	 * @param startPoint
	 *            Start point of the animation.
	 * @param endPoint
	 ...
	 *            The values on the y axis.
	 */
	private void getHideXY(float[] startPoint, float[] endPoint, float[] xs,
			float[] ys) {
		if (boomType.equals(BoomType.LINE)) {
			float x1 = startPoint[0];
			float y1 = startPoint[1];
	...
	private void startHideAnimations() {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			if (hideOrderType.equals(OrderType.DEFAULT)) {
				for (int i = 0; i < buttonNum; i++) {
					setHideAnimation(dots[i], circleButtons[i],
							endLocations[i], startLocations[i], i);
				}
			} else if (hideOrderType.equals(OrderType.REVERSE)) {
				for (int i = 0; i < buttonNum; i++) {
					setHideAnimation(dots[i], circleButtons[i],
							endLocations[i], startLocations[i], buttonNum - i
									- 1);
				}
				...
				while (true) {
					...
					if (!used[i]) {
						...
						setHideAnimation(dots[count], circleButtons[count],
								endLocations[count], startLocations[count], i);
			...
			if (hideOrderType.equals(OrderType.DEFAULT)) {
				for (int i = 0; i < buttonNum; i++) {
					setHideAnimation(bars[i], hamButtons[i], endLocations[i],
							startLocations[i], i);
				}
			} else if (hideOrderType.equals(OrderType.REVERSE)) {
				for (int i = 0; i < buttonNum; i++) {
					setHideAnimation(bars[i], hamButtons[i], endLocations[i],
							startLocations[i], buttonNum - i - 1);
				}
			} else if (hideOrderType.equals(OrderType.RANDOM)) {
				Random random = new Random();
				boolean[] used = new boolean[buttonNum];
				...
				while (true) {
					...
					if (!used[i]) {
						...
						setHideAnimation(bars[count], hamButtons[count],
								endLocations[count], startLocations[count], i);
	 ...
	 */
	@SuppressLint("NewApi")
	public void setHideAnimation(final View dot, final View button,
			int[] startLocation, int[] endLocation, int index) {
		...
		ObjectAnimator xAnimator = ObjectAnimator.ofFloat(button, "x", xs)
				.setDuration(duration);
		xAnimator.setStartDelay(index * delay);
		xAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(hideMoveEaseType));
		xAnimator.start();

		ObjectAnimator yAnimator = ObjectAnimator.ofFloat(button, "y", ys)
				.setDuration(duration);
		...
		ObjectAnimator scaleXAnimator = ObjectAnimator.ofFloat(button,
				"scaleX", 1f, scaleW).setDuration(duration);
		scaleXAnimator.setStartDelay(index * delay);
		scaleXAnimator.setInterpolator(InterpolatorFactory
				.getInterpolator(hideScaleEaseType));
		scaleXAnimator.start();

		ObjectAnimator scaleYAnimator = ObjectAnimator.ofFloat(button,
				"scaleY", 1f, scaleH).setDuration(duration);
		...
		if (button instanceof CircleButton) {
			ObjectAnimator rotateAnimator = ObjectAnimator.ofFloat(
					((CircleButton) button).getFrameLayout(), "rotation", 0,
					-rotateDegree).setDuration(duration);
			rotateAnimator.setStartDelay(index * delay);
			rotateAnimator.setInterpolator(InterpolatorFactory
					.getInterpolator(hideRotateEaseType));
			rotateAnimator.start();
		}
	...
	public void lightAnimationLayout() {
		ObjectAnimator objectAnimator = ObjectAnimator.ofInt(animationLayout,
				"backgroundColor", dimType.value, DimType.DIM_0.value)
				.setDuration(duration + delay * (buttonNum - 1));
		objectAnimator.setEvaluator(new ArgbEvaluator());
		objectAnimator.addListener(new AnimatorListenerAdapter() {
			@Override
	...
	/**
	 * Set auto dismiss. If the boom menu button is auto dismiss, user can click
	 * one of the sub buttons to dismiss the boom menu botton.
	 *
	 * @param autoDismiss
	 */
	...
	/**
	 * Set cancelable. If the boom menu button is cancelable, user can click the
	...
	public ImageView[] getImageViews() {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			for (int i = 0; i < buttonNum; i++)
				imageViews[i] = circleButtons[i].getImageView();
		} else if (buttonType.equals(ButtonType.HAM)) {
			for (int i = 0; i < buttonNum; i++)
				imageViews[i] = hamButtons[i].getImageView();
		}
		return imageViews;
	}
	...
	public TextView[] getTextViews() {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			for (int i = 0; i < buttonNum; i++)
				textViews[i] = circleButtons[i].getTextView();
		} else if (buttonType.equals(ButtonType.HAM)) {
			for (int i = 0; i < buttonNum; i++)
				textViews[i] = hamButtons[i].getTextView();
		}
		return textViews;
	}
	...
	public void setOnSubButtonClickListener(
			OnSubButtonClickListener onSubButtonClickListener) {
	...
	public void setBoomButtonBackgroundColor(int pressedColor, int normalColor) {
		Util.getInstance().setCircleButtonStateListDrawable(frameLayout,
				boomButtonRadius, pressedColor, normalColor);
	}

	/**
	 * Set the offset of the shadow layouts of sub buttons. If xOffset is 0 and
	 * yOffset is 0, then the shadow layout is at the center.
	 *
	 * @param xOffset
	 *            In pixels.
	 * @param yOffset
	...
	/**
	 * Set the offset of the shadow layouts of boom menu button. If xOffset is 0
	 * and yOffset is 0, then the shadow layout is at the center.
	 *
	 * @param xOffset
	 *            In pixels.
	 * @param yOffset
	...
	/**
	 * Set the color of all textviews in the sub buttons corresonding to the
	 * array.
	 *
	 * @param colors
	 */
	public void setTextViewColor(int[] colors) {
		...
		if (buttonType.equals(ButtonType.CIRCLE)) {
			...
			for (int i = 0; i < length; i++)
				hamButtons[i].getTextView().setTextColor(colors[i]);
		}
	}
	...
	public void onClick(int index) {
		if (!state.equals(StateType.OPEN))
			return;
		if (onSubButtonClickListener != null)
			onSubButtonClickListener.onClick(index);
		if (autoDismiss && !animationPlaying)
			startHideAnimations();
	}
